...
public class ClassicCvDocument : IDocument
{
    ...
    private const string BodyFont = "Inter";
    private const string NameFont = "Georgia";
    ...
    public ClassicCvDocument(CvData data)
    {
        _data = data;
    }
    ...
    public void Compose(IDocumentContainer container)
    {
        container.Page(page =>
        {
            ...
            page.Content().Column(column =>
            {
                ...
                // Skills — comma-separated on a single line
                if (_data.Skills is { Count: > 0 })
                {
                    ComposeSectionTitle(column, "Skills");
                    column.Item().PaddingBottom(10)
                        .Text(string.Join(", ", _data.Skills))
                        .FontFamily(BodyFont).FontSize(10).FontColor(BodyColor);
                }
    ...
    private void ComposeHeader(ColumnDescriptor column)
    {
        ...
        column.Item().PaddingTop(4).AlignCenter().Row(row =>
        {
            ...
            if (_data.Candidate.Phone is not null)
                parts.Add(_data.Candidate.Phone);

            if (_data.Candidate.Location is not null)
                parts.Add(_data.Candidate.Location);
        ...
        column.Item().PaddingBottom(12).Column(item =>
        {
            item.Item().Row(r =>
            {
                r.RelativeItem().Text(exp.Role)
                    .FontFamily(BodyFont).FontSize(11).Bold().FontColor(TitleColor);
                r.AutoItem().Text(dateStr)
                    .FontFamily(BodyFont).FontSize(9).FontColor(GrayText);
            });
            item.Item().PaddingTop(1).Text(exp.Company)
                .FontFamily(BodyFont).FontSize(10).Italic().FontColor(GrayText);
                    ...
                    foreach (var b in exp.Bullets)
                    {
                        bullets.Item().PaddingBottom(3).Row(row =>
                        {
                            ...
                            row.RelativeItem().Text(b)
                                .FontFamily(BodyFont).FontSize(10).FontColor(BodyColor).LineHeight(1.5f);
                        });
                    }
                });
        });
    ...
    private static void ComposeEducation(ColumnDescriptor column, CvEducation edu)
    {
        column.Item().PaddingBottom(10).Column(item =>
        {
            item.Item().Text(edu.Degree)
                .FontFamily(BodyFont).FontSize(11).Bold().FontColor(TitleColor);
            item.Item().PaddingTop(1).Text(edu.Institution)
                .FontFamily(BodyFont).FontSize(10).Italic().FontColor(GrayText);
            item.Item().PaddingTop(1).Text(edu.Year)
                .FontFamily(BodyFont).FontSize(9).FontColor(GrayText);
        });
    }
}
