...
public class ModernCvDocument : IDocument
{
    ...
    public void Compose(IDocumentContainer container)
    {
        container.Page(page =>
        {
            ...
            // Using page.Header() ensures the header takes exactly what it needs
            page.Header().Background(HeaderBg).Padding(25).Column(column =>
            {
                ...
                column.Item().PaddingTop(4).Row(row =>
                {
                    ...
                    {
                        row.AutoItem().PaddingHorizontal(8).Text("·")
                            .FontFamily(FontName).FontSize(10).FontColor(HeaderLightText);
                        row.AutoItem().Text(_data.Candidate.Location)
                            .FontFamily(FontName).FontSize(10).FontColor(HeaderLightText);
                    }
                    ...
                    if (_data.Candidate.Phone is not null)
                    {
                        row.AutoItem().PaddingHorizontal(8).Text("·")
                            .FontFamily(FontName).FontSize(10).FontColor(HeaderLightText);
                        row.AutoItem().Text(_data.Candidate.Phone)
                            .FontFamily(FontName).FontSize(10).FontColor(HeaderLightText);
                    }
                });
            });

            // page.Content() takes the remaining vertical space of the entire A4 page
    ...
    private void ComposeLeftColumn(ColumnDescriptor column)
    {
            ...
            foreach (var skill in _data.Skills)
            {
                column.Item().PaddingBottom(4).Row(row =>
                {
                    row.AutoItem().PaddingRight(5).Text("•")
                        .FontFamily(FontName).FontSize(9).FontColor(BodyColor);
            ...
            foreach (var edu in _data.Education)
            {
                column.Item().PaddingBottom(10).Column(e =>
                {
                    e.Item().Text(edu.Degree)
                        .FontFamily(FontName).FontSize(10).Bold().FontColor(TitleColor);
                    e.Item().PaddingTop(1).Text(edu.Institution)
                        .FontFamily(FontName).FontSize(9).Italic().FontColor(GrayText);
                    e.Item().PaddingTop(1).Text(edu.Year)
                        .FontFamily(FontName).FontSize(9).FontColor(GrayText);
                });
            }
        }
        ...
        if (_data.Languages is { Count: > 0 })
        {
            ...
            {
                column.Item().PaddingBottom(3).Text(lang)
                    .FontFamily(FontName).FontSize(9).FontColor(BodyColor);
            }
        }
    }
    ...
    private void ComposeRightColumn(ColumnDescriptor column)
    {
        ...
        if (_data.Certifications is { Count: > 0 })
        {
            ...
            foreach (var cert in _data.Certifications)
            {
                ...
                {
                    row.AutoItem().PaddingRight(5).Text("•")
                        .FontFamily(FontName).FontSize(9).FontColor(AccentColor);
    ...
    private static void ComposeSectionTitle(ColumnDescriptor column, string title)
    {
        column.Item().PaddingBottom(6).Column(titleCol =>
        {
            titleCol.Item().Text(title)
                .FontFamily(FontName).FontSize(12).Bold().FontColor(TitleColor);
        ...
        column.Item().PaddingBottom(12).Column(item =>
        {
            item.Item().Row(r =>
            {
                r.RelativeItem().Text(exp.Role)
                    .FontFamily(FontName).FontSize(11).Bold().FontColor(TitleColor);
                r.AutoItem().Text(dateStr)
                    .FontFamily(FontName).FontSize(9).FontColor(GrayText);
            });
            item.Item().PaddingTop(1).Text(exp.Company)
                .FontFamily(FontName).FontSize(10).Italic().FontColor(GrayText);
                ...
                item.Item().PaddingTop(4).Column(bullets =>
                {
                    foreach (var b in exp.Bullets)
                    {
                        bullets.Item().PaddingBottom(3).Row(row =>
                        {
                            row.AutoItem().PaddingRight(5).Text("•")
                                .FontFamily(FontName).FontSize(9).FontColor(BodyColor);
                            row.RelativeItem().Text(b)
                                .FontFamily(FontName).FontSize(9).FontColor(BodyColor).LineHeight(1.5f);
                        });
                    }
                });
        });
    ...
    private static void ComposeProject(ColumnDescriptor column, CvProject prj)
    {
        column.Item().PaddingBottom(10).Column(item =>
        {
            ...
            if (!string.IsNullOrWhiteSpace(prj.Description))
                item.Item().PaddingTop(1).Text(prj.Description)
                    .FontFamily(FontName).FontSize(9).Italic().FontColor(GrayText);
            ...
            item.Item().Row(r =>
            {
                ...
                if (!string.IsNullOrWhiteSpace(act.Role))
                    r.AutoItem().Text($"  —  {act.Role}")
                        .FontFamily(FontName).FontSize(10).FontColor(GrayText);
            });
            if (!string.IsNullOrWhiteSpace(act.Description))
                item.Item().PaddingTop(2).Text(act.Description)
                    .FontFamily(FontName).FontSize(9).FontColor(BodyColor).LineHeight(1.5f);
        });
    }
}
