...
def main() -> int:
    parser = argparse.ArgumentParser(description="Manual test for the LinkedIn jobs LangGraph agent")
    parser.add_argument("--base-url", default="http://localhost:8000", help="Agents API base url")
    parser.add_argument("--keywords", default="software engineer", help="LinkedIn job keywords")
    parser.add_argument("--location", default="Morocco", help="LinkedIn location filter")
    parser.add_argument("--limit", type=int, default=10, help="Max number of jobs to return")
    parser.add_argument("--timeout", type=int, default=180, help="HTTP timeout in seconds")
    args = parser.parse_args()

    url = f"{args.base_url.rstrip('/')}/linkedin-jobs/search"
