...
describe('ProfileFeatureService (State & Signals)', () => {
  ...
  it('devrait mettre à jour les infos personnelles (savePersonalInfo)', async () => {
    const info: PersonalInfo = { firstName: 'Jane', lastName: 'Doe', email: 'jane@test.com', phone: '', city: '', country: '', jobTitle: '', linkedinUrl: '', githubUrl: '', photoUrl: null, useAsHeadline: true, address: '' };
  ...
  it('devrait ajouter une expérience et recharger le profil (addExperience)', async () => {
    const exp: Experience = { id: '', company: 'Tech', title: 'Dev', startDate: '2023-01', endDate: '', description: 'Code', city: 'Paris', type: 'CDI', current: true };
  ...
  it('devrait ajouter un projet (addProject)', async () => {
    const loadSpy = vi.spyOn(service, 'loadProfile').mockResolvedValue(undefined);
    const proj: Project = { id: '', title: 'A', description: 'B', stack: [], githubUrl: '', demoUrl: '', isUniversity: false };
  ...
  it('devrait mettre à jour un projet (updateProject)', async () => {
    const loadSpy = vi.spyOn(service, 'loadProfile').mockResolvedValue(undefined);
    const proj: Project = { id: '4', title: 'A', description: 'B', stack: [], githubUrl: '', demoUrl: '', isUniversity: false };
